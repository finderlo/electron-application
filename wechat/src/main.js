...
class ElectronicWeChat {
    ...
    initIPC() {
        ...
        ipcMain.on('reload', (event, repetitive) => {
            if (repetitive) {
                this.wechatWindow.loginState.current = this.wechatWindow
                    .loginState.NULL;
        ...
        ipcMain.on('update', (event, message) => {
            ...
            updateHandler.checkForUpdate(`v${app.getVersion()}`,
                false);
        });
    };
...
const shouldQuit = app.makeSingleInstance((commandLine, workingDirectory) => {
    ...
    if (mainApp) {
        mainApp.wechatWindow.wechatWindow.restore()
        mainApp.wechatWindow.wechatWindow.show()
        mainApp.wechatWindow.wechatWindow.focus()
    }
});
